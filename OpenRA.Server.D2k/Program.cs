...
namespace OpenRA.Server.D2k
{
	class Program
	{
		...
		static void Run(string[] args)
		{
			...
			var modID = "d2k";
			var explicitModPaths = new[] { modID };
			...
			while (true)
			{
				var assemblyList = new[]
				{
					...
					typeof(ModContent).Assembly,
					typeof(AudLoader).Assembly,
					typeof(Mods.D2k.PackageLoaders.D2kSoundResourcesLoader).Assembly,
				};

				// HACK: The engine code *still* assumes that Game.ModData is set
				var modData = Game.ModData = new ModData(mods[modID], mods, assemblyList);
